	...
	if(component_id == 2 || (component_id == 1 && janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_RTCPMUX) && janus_is_rtcp(buf))) {
		...
		if(!component->dtls || !component->dtls->srtp_valid || !component->dtls->srtp_in) {
			...
			if(res != srtp_err_status_ok) {
				...
				JANUS_LOG(LOG_DBG, "%p remote SSRC: video=%u, audio=%u, video:%i, audio:%i\n ", stream, stream->video_ssrc_peer, stream->audio_ssrc_peer, &stream->video_ssrc_peer_bye_received, &stream->audio_ssrc_peer_bye_received);
				if(janus_rtcp_has_bye(buf, buflen, stream->video_ssrc_peer, stream->audio_ssrc_peer, stream->video_ssrc_peer_bye_received, stream->audio_ssrc_peer_bye_received)) {
...
int janus_ice_setup_local(janus_ice_handle *handle, int offer, int audio, int video, int data, int bundle, int rtcpmux, int trickle) {
	...
	if(audio) {
		...
		audio_stream->audio_ssrc_peer_bye_received = FALSE;
		...
		audio_stream->video_ssrc_peer_bye_received = FALSE;
	...
	if(video && (!audio || !janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_BUNDLE))) {
		...
		video_stream->video_ssrc_peer_bye_received = FALSE;
		...
		video_stream->audio_ssrc_peer_bye_received = FALSE;
	...
	if(data && ((!audio && !video) || !janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_BUNDLE))) {
		...
		data_stream->audio_ssrc_peer_bye_received = FALSE;
		data_stream->video_ssrc_peer_bye_received = FALSE;
		/* FIXME By default, if we're being called we're DTLS clients, but this may be changed by ICE... */
